      ...
      <div className="space-y-5">
        <SectionCard title="Datos generales">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormField label="Volumen objetivo (L)" error={form.formState.errors.targetVolumeL?.message}>
              <Input type="number" step="0.1" {...form.register("targetVolumeL")} />
            </FormField>
            <FormField label="Eficiencia de macerado (%)" error={form.formState.errors.efficiencyPct?.message}>
              <Input type="number" step="1" {...form.register("efficiencyPct")} />
            </FormField>
          </div>
        </SectionCard>

        <SectionCard title="Objetivos" description="Los números a los que apunta esta receta.">
            ...
            {fermentables.fields.map((field, idx) => (
              <Card key={field.id} className="space-y-3 border-[var(--color-border-hairline)] bg-[var(--color-bg-elevated)] p-3.5">
                <div className="flex items-start gap-2">
                  <Input placeholder="Nombre (ej: Pale Ale Malt)" className="flex-1" {...form.register(`fermentables.${idx}.name`)} />
                  <Button type="button" variant="ghost" size="icon-sm" onClick={() => fermentables.remove(idx)}>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <Input type="number" step="0.01" placeholder="kg" {...form.register(`fermentables.${idx}.amountKg`)} />
                  <Input type="number" step="1" placeholder="PPG potencial" {...form.register(`fermentables.${idx}.potentialPpg`)} />
                  <Input type="number" step="1" placeholder="Color °L" {...form.register(`fermentables.${idx}.colorLovibond`)} />
                </div>
        ...
        >
          ...
          <div className="space-y-3">
            {hops.fields.map((field, idx) => (
              <Card key={field.id} className="space-y-3 border-[var(--color-border-hairline)] bg-[var(--color-bg-elevated)] p-3.5">
                <div className="flex items-start gap-2">
                  <Input placeholder="Nombre (ej: Cascade)" className="flex-1" {...form.register(`hops.${idx}.name`)} />
                  <Button type="button" variant="ghost" size="icon-sm" onClick={() => hops.remove(idx)}>
                    <Trash2 className="h-4 w-4 text-[var(--color-danger)]" />
                  </Button>
                </div>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <Input type="number" step="0.1" placeholder="%AA" {...form.register(`hops.${idx}.alphaAcidPct`)} />
                  <Input type="number" step="1" placeholder="gramos" {...form.register(`hops.${idx}.amountG`)} />
                  ...
                  <Input type="number" step="1" placeholder="minutos" {...form.register(`hops.${idx}.timeMin`)} />
                </div>
        ...
        <SectionCard
          title="Adjuntos"
          description="Miel, mosto de uva, especias, etc."
          actions={
            <Button
              type="button"
              variant="secondary"
        ...
        >
          ...
          <div className="space-y-3">
            {adjuncts.fields.map((field, idx) => (
              <Card key={field.id} className="space-y-3 border-[var(--color-border-hairline)] bg-[var(--color-bg-elevated)] p-3.5">
                <div className="flex items-start gap-2">
                  <Input placeholder="Nombre (ej: Miel)" className="flex-1" {...form.register(`adjuncts.${idx}.name`)} />
                  <Button type="button" variant="ghost" size="icon-sm" onClick={() => adjuncts.remove(idx)}>
                    <Trash2 className="h-4 w-4 text-[var(--color-danger)]" />
                  </Button>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <Input type="number" step="0.01" placeholder="kg" {...form.register(`adjuncts.${idx}.amountKg`)} />
                  <Input type="number" step="1" placeholder="PPG potencial" {...form.register(`adjuncts.${idx}.potentialPpg`)} />
                  <Input placeholder="Momento de adición" {...form.register(`adjuncts.${idx}.timing`)} />
                </div>
        ...
        <div className="sticky bottom-20 z-10 rounded-[var(--radius-lg)] border border-[var(--color-primary)]/30 bg-[var(--color-bg-overlay)]/95 p-4 backdrop-blur md:static md:bottom-auto">
